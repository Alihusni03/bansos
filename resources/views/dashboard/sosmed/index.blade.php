@extends('dashboard.layouts.main')

    @section('container')
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Kelola sosmed</h1>
      </div>

      @if(session()->has('success'))
        <div class="alert alert-success" role="alert">
          {{ session('success') }}
        </div>
      @endif

      <div class="table-responsive">
        @if ($sosmeds->isEmpty())
        <a href="/dashboard/sosmed/create" class="btn btn-success mb-3">Tambah sosmed</a>
        @endif
        <table class="table table-striped table-sm">
          <thead>
            <tr>
              <th scope="col">No</th>
              <th scope="col">Facebook</th>
              <th scope="col">Twitter</th>
              <th scope="col">Youtube</th>
              <th scope="col">Instagram</th>
              <th scope="col">WhatsApp</th>
              <th scope="col">Action</th>
            </tr>
          </thead>
          <tbody>
          @foreach ($sosmeds as $sosmed)
            <tr>
              <td>{{ $loop->iteration }}</td>
              <td>{{ $sosmed->fb }}</td>
              <td>{{ $sosmed->tw }}</td>
              <td>{{ $sosmed->yt }}</td>
              <td>{{ $sosmed->ig }}</td>
              <td>{{ $sosmed->wa }}</td>
              <td>
                <!-- <a href="/dashboard/sosmed/{{$sosmed->id}}" class="badge bg-info"><span data-feather="eye"></span></a> -->
                <a href="/dashboard/sosmed/{{$sosmed->id}}/edit" class="badge bg-warning"><span data-feather="edit"></span></a>
                <form action="/dashboard/sosmed/{{$sosmed->id}}" method="post" class="d-inline">
                  @method('delete')
                  @csrf
                  <button class="badge bg-danger border-0" onclick="return confirm('Apakah kamu yakin?')"><span data-feather="trash"></button>
                </form>
              </td>
            </tr>
            @endforeach
          </tbody>
        </table>
      </div>
    @endsection
